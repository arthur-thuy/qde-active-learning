    ...
    def get_subsampled_race_dataset(
        ...
        if balanced_sampling:
            df_train = pd.concat(
                [
                    df_train[df_train[DIFFICULTY] == 0].sample(
                        training_size, random_state=random_state
                    ),
                    df_train[df_train[DIFFICULTY] == 1].sample(
                        training_size, random_state=random_state
                    ),
            ...
            df_train = df_train.sample(training_size, random_state=random_state)
        df_train.to_csv(
            os.path.join(output_data_dir, f"race_{training_size}_{TRAIN}.csv"),
            index=False,
        )
        df_dev.to_csv(
            os.path.join(output_data_dir, f"race_{training_size}_{DEV}.csv"),
            index=False,
        )
        df_test.to_csv(
            os.path.join(output_data_dir, f"race_{training_size}_{TEST}.csv"),
            index=False,
        )
        subsampled_dataset[TRAIN] = df_train.copy()
        subsampled_dataset[DEV] = df_dev.copy()
